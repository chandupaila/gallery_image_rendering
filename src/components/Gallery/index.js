...
class Gallery extends Component {
  state = {displayImageId: imagesList[0].id}
  ...
  renderSelectedImage = () => {
    ...
    const {imageUrl, imageAltText} = imagesList[displayImageId]
    ...
    return (
      <div className="bg-container">
        <div className="inner-container">
          <div className="image-container">{this.renderSelectedImage()}</div>
          ...
          <ul className="unorderList">
            {imagesList.map(eachItem => (
              <ThumbnailItem
                ...
                isActive={displayImageId === eachItem.id}
              />
            ))}
          </ul>
        </div>
